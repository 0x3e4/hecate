...
| `/scans/targets/:targetId` | `ScanTargetDetailPage` | Per-target overview (deep-linkable): metadata, severity rollup, auto-scan diagnostics, scan history, top findings, quick actions (rescan / check / delete), shield-badge copy, and the per-target **Write protection** set/clear control (🔒 badge). Registered before `/scans/:scanId`. |
