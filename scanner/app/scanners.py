...
def setup_auth(auth_entries: str) -> None:
    ...
    Examples:
      SCANNER_AUTH=ghcr.io:gitea_token
...
async def _run_command(cmd: list[str], timeout: int = 600) -> tuple[str, str, int]:
    ...
    Logs start/end/timeout/exception so operators can see — from the scanner
    sidecar's stdout — which subprocess is running, what timeout it was
    given (resolves any doubt about whether `<SCANNER>_TIMEOUT_SECONDS`
    actually reached the container), and how long it took to finish.
    """
    name = os.path.basename(cmd[0]) if cmd else "<empty>"
    started_at = time.monotonic()
    logger.info("subprocess.start name=%s timeout=%ds", name, timeout)
    # Full argv at DEBUG so operators can reproduce a scanner invocation
    ...
    elapsed = time.monotonic() - started_at
    logger.info(
        "subprocess.finished name=%s rc=%d duration=%.1fs stderr_bytes=%d",
        name, rc, elapsed, len(stderr),
    )
...
async def _run_devskim(
    ...
    try:
        ...
        # DevSkim does pure pattern-matching and walks every byte; trimming
        # vendored / generated noise is the biggest single lever on .NET
        # codebases. Other scanners (trivy/grype/syft) skip these via
        # ecosystem rules and don't need the help.
        cmd = [
            "devskim", "analyze",
            "-I", scan_dir,
            "-O", out_path,
            ...
            "--ignore-globs", (
                ...
                "**/wwwroot/lib/**,"     # vendored JS/CSS in ASP.NET
                "**/.vs/**,"             # Visual Studio cache
                "**/TestResults/**,"     # test report dumps
                "**/*.Designer.cs"       # autogenerated WinForms / Resx designer files
            ),
        ]
