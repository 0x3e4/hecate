...
| `/scans/targets/*` | `ScanTargetDetailPage` | Per-target overview (deep-linkable; splat route registered before `/scans/:scanId` — pasted un-encoded or scheme-less target URLs resolve server-side and the address bar is replaced with the canonical encoded form). Card-based layout matching the scan detail page: in-header back link + `↗`, severity rollup, scanner chips, auto-scan `/check` diagnostics, quick actions (rescan / run check / copy badge / delete), top findings, an **SBOM changes** card (added / updated `old → new` / removed components between the two latest completed scans, headed by the latest scan's time + commit short-hash; baseline line when only one scan exists), and **server-paginated** scan history (Load more / Show all, horizontal-scroll on mobile). A 🔒 badge shows when the target is write-protected (passwords are managed in System → Access Control). Transient messages use the shared bottom-right `Toast`. |
