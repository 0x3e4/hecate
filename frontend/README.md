...
| `/vulnerabilities` | `VulnerabilityListPage` | Paginated list with full-text, vendor, product, version filters and an advanced filter set (severity, CVSS vector, EPSS, CWE, sources, time range). Three-way search-mode pill above the input: **Keyword** (full-text), **DQL** (existing query syntax), **Regex** (Lucene `regexp` over a curated field set — summary, title, vendors, products, aliases, ID, versions, references, CWEs; keep `REGEX_SEARCH_FIELDS` here in sync with the backend tuple in `vulnerability_service.py`). Saved searches preserve the originating mode via `queryMode` + `regexQuery`. Clicking a version chip on a row appends the vendor and product slugs alongside the version, and uses `affectedVersion:` (range-covering match) for concrete releases, falling back to a literal `product_versions:` term for advisory range strings that aren't a single version (`isConcreteVersion`). |
