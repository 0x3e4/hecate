...
│   │                          target-group roll-up, history (since filter), SBOM diff walking back to the last scan pair
│   │                          whose SBOM actually changed (GET /scans/targets/{id:path}/sbom-diff), findings (?includeDismissed; advisory unaffected/
...
| --- | --- |
| `VulnerabilityService` | Search, refresh, lookup. Search supports full-text / DQL **and** regex (`regexQuery` runs Lucene `regexp` over the curated `REGEX_SEARCH_FIELDS` tuple — summary, title, vendors, products, aliases, ID, versions, references, CWEs). Saved searches persist `regexQuery` + `queryMode` (`keyword` / `dql` / `regex`) so the UI can restore the mode. DQL additionally supports `affectedVersion:X` — lifted out of the query string by `_extract_affected_version_terms` (replaced with Lucene's `*:*` so the boolean structure stays valid) and applied as a nested `versionRanges` filter by `_version_range_clause`, scoped to the vendor/product the query already narrows to. The `product:vendor/product version:X` shorthand resolves through the same clause. |
...
| `ScanService` | SCA scan orchestration (concurrency limiting, resource gating, SBOM import, AI analysis, per-scan SBOM dedup via `unique_component_keys → sbom_component_count`, `backfill_sbom_component_count_v2` startup backfill). `delete_target` deletes the target row synchronously and returns immediately, then fans out the scans/findings/SBOM/layer-analysis cleanup as a background `asyncio.Task`; `reconcile_orphaned_target_data()` runs once at startup to sweep up child rows left behind by a crash mid-cascade. `get_target_sbom_diff` walks back through up to `SBOM_DIFF_LOOKBACK_LIMIT` (20) recent completed scans to find the last pair whose SBOM actually differs, so a run of no-op auto-rescans doesn't hide an earlier real change. |
...
| `InventoryService` / `inventory_matcher` | CRUD + matching with a pure-function CPE version-range matcher (self-contained version comparator). The matcher is uniformly **fail-closed for version-less references**: a bare `vendor:product:*` / `:-:` CPE with no bounds and broad range strings (`>=0`, `*`, `-`, empty) never match a specific installed version. `_BRANCH_SCOPED_PRODUCTS` (currently NetScaler ADC / Gateway) treats a range clause as an independent per-branch build counter instead of one linear scale, so a comma-laced "branch, build" version (e.g. `14.1, 66.59`) is evaluated against its own branch's threshold rather than colliding with an unrelated branch's range; `parse_version` fails closed on any trailing non-numeric garbage instead of silently truncating. `_parse_range_clauses` normalises EUVD's typographic / implicit-lower-bound range shapes (`4.7 ≤4.7.30`, `0 ≤2.4.7`, `n/a ≤≤ 1.50.2`) into the ASCII clause form before scanning — without it the scanner read only the leading token and treated a whole-release-line range as the *exact* version `4.7`; `n/a` and friends are broad sentinels. The same parser backs the `versionRanges` index. Auto-links items to an endoflife.date product (`eol_product`) on create / product change. |
...
**`versionRanges` (nested)** — flat half-open numeric intervals `{vendorSlug, productSlug, startNumeric, endNumeric}` (start inclusive, end exclusive) that back the `affectedVersion:` DQL field and the range half of the version filter. Built by `app/services/version_ranges.py` out of all three range sources on a record (`impactedProducts[].versions` range strings, `cpeConfigurations` bounds, concrete flat `cpes`) using the base-10000 encoding from `_encode_version_numeric`, so "which advisories affect the version I run?" collapses to one nested `startNumeric <= enc(V) < endNumeric` clause. Needed because `productVersions` only holds the boundaries an advisory *names* — an advisory for "7.0.x before 7.0.2" stores `7.0` and `7.0.2`, never `7.0.1`. Fail-closed like `inventory_matcher`: broad sentinels (`*`, `-`, `n/a`, `>=0`) and version-less wildcard CPEs are not indexed, and an unencodable bound drops the range rather than widening it. Derived at index time inside `VulnerabilityDocument.opensearch_document()` (not persisted to MongoDB), so every write path populates it and `reindex-opensearch` backfills the corpus; the mapping is added to existing indices additively, without recreation.

**Exact-match routing for identifier fields** — `vendorSlugs` / `productSlugs` / `productVersions` / `productVersionIds` / `sourceNames` are declared `keyword`, but indices created before those declarations had them dynamically mapped as analyzed `text` + `.keyword`. There `vendorSlugs:"wordpress"` matches any slug *containing* the token (`dokan-wordpress-plugin` → `[dokan, wordpress, plugin]`). Mapping types can't be changed in place, so `refresh_exact_field_paths()` in `app/db/opensearch.py` reads the live mapping at startup and `_route_exact_dql_fields` (last step of `_translate_dql_fields`) rewrites those clauses to whichever path is exact on that index. No-op when correctly mapped; wildcards keep working. Display-name fields (`vendors` / `products`) stay analyzed on purpose.
